...
class GloveEmbedding:
    ...
    def load_embeddings(self) -> gensim.models.KeyedVectors:
        ...
        all_vectors = {word: torch.tensor(self.glove_model[word]) for word in self.glove_model.key_to_index}
        unk_vector = torch.mean(torch.stack(list(all_vectors.values())), dim=0) if all_vectors else torch.zeros(
            self.glove_model.vector_size)
...
class Text2DP(Text2Graph):
    ...
    def __init__(self, lang="english", max_num_nodes=1000):
        ...
        self.embeddings_path = {
            ...
            "portuguese": "data/external/embeddings/ptwiki_20180420_100d.txt"
        }
    ...
    def parse_document(self, text: str, preprocessing_fn: Optional[Callable] = None) -> nx.MultiDiGraph:
        """
    ...
    def _add_dependency_edges(self, graph: nx.MultiDiGraph, doc: spacy.tokens.Doc) -> None:
        """
        Add dependency parsing edges to the graph.

        Args:
            graph (nx.MultiDiGraph): The graph to which edges are added.
            doc (spacy.tokens.Doc): The spaCy processed document.
        """
        for token in doc:
            if len(token.text.strip()) > 0 and token.dep_ not in self.pos_tag_to_ignore:
                add_new_relation_to_graph(
                    graph,
                    token.head.text.strip(),
                    token.text.strip(),
    ...
    def _add_sequential_edges(self, graph: nx.MultiDiGraph, doc: spacy.tokens.Doc) -> None:
        ...
        considering POS tags to ignore.

        Args:
            graph (nx.MultiDiGraph): The graph to which edges are added.
            doc (spacy.tokens.Doc): The spaCy processed document.
        """
        # Filter out tokens based on POS tags to ignore
        filtered_tokens = [
            token for token in doc if token.dep_ not in self.pos_tag_to_ignore
        ]

        # Add sequential edges only between the valid tokens
...
class Text2GraphDataset:
    ...
    def parse(self, corpus=None):
        ...
        for proc_number, label, text in tqdm(corpus, desc="Parsing batch"):
            graph = self.text_to_graph_parser.parse_document(text.strip().lower(), preprocessing_fn=__preprocessing)
    ...
    def build_pyg_dataset_individual(self, input_filename=None, parsed_graphs=None, output_path=None, batch_index=0):
        ...
        for i, (proc_number, label, text, graph) in tqdm(enumerate(self.parsed_graphs)):
            try:
